<div>
    <x-migas :items="['Noticias' => null]" raiz="admin.panel" etiqueta-raiz="Panel principal" class="mb-6" />

    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-texto">Noticias</h1>
            <p class="mt-1 text-sm text-texto-secundario">Avisos y comunicados institucionales del OICM.</p>
        </div>
    ...
    @if ($noticias->isEmpty())
        <x-vacio titulo="Sin noticias que coincidan" descripcion="Ajusta la búsqueda o los filtros para ver más resultados." />
    @else
        <x-tarjeta class="!p-0">
                ...
                <table class="w-full text-left text-sm">
                    <thead class="border-b border-borde text-xs uppercase tracking-wide text-texto-secundario">
                        <tr>
                            ...
                            <th class="px-6 py-3 font-semibold">Autor</th>
                    ...
                    <tbody class="divide-y divide-borde">
                        @foreach ($noticias as $noticia)
                            <tr wire:key="noticia-{{ $noticia->id }}">
                                <td class="px-6 py-4">
                                    <div class="font-medium text-texto">{{ $noticia->titulo }}</div>
                                </td>
                                <td class="px-6 py-4 text-texto-secundario">{{ $noticia->autor?->name ?? '—' }}</td>
                                <td class="px-6 py-4 text-texto-secundario">
                                    {{ $noticia->publicado_en?->format('d/m/Y H:i') ?? '—' }}
                                </td>
                                <td class="px-6 py-4">
                                    @if ($noticia->estatus === 'publicada' && $noticia->publicado_en?->isFuture())
                                        <x-badge variante="advertencia" :punto="true">Programada</x-badge>
                                    @elseif ($noticia->estatus === 'publicada')
                                    ...
                                    <div
                                        ...
                                        x-data="{
                                            async confirmarYEjecutar(metodo, titulo, texto) {
                                                ...
                                                if (confirmado) { $wire[metodo]({{ $noticia->id }}); }
                                            },
                                        }"
                                    >
                                        <x-tooltip texto="Editar">
                                            <button type="button" wire:click="editar({{ $noticia->id }})" class="rounded-full p-2 text-texto-secundario hover:bg-primario-claro hover:text-primario" aria-label="Editar {{ $noticia->titulo }}">
                                        ...
                                        <x-tooltip texto="{{ $noticia->estatus === 'publicada' ? 'Regresar a borrador' : 'Publicar' }}">
                                            <button type="button" x-on:click="confirmarYEjecutar('alternarEstatus', '{{ $noticia->estatus === 'publicada' ? '¿Regresar a borrador' : '¿Publicar' }} {{ $noticia->titulo }}?', 'El cambio se refleja de inmediato en la vista pública.')" class="rounded-full p-2 text-texto-secundario hover:bg-advertencia-suave hover:text-advertencia" aria-label="{{ $noticia->estatus === 'publicada' ? 'Regresar a borrador' : 'Publicar' }} {{ $noticia->titulo }}">
                                        ...
                                        <x-tooltip texto="Eliminar">
                                            <button type="button" x-on:click="confirmarYEjecutar('eliminar', '¿Eliminar {{ $noticia->titulo }}?', 'Esta acción no se puede deshacer.')" class="rounded-full p-2 text-texto-secundario hover:bg-error-suave hover:text-error" aria-label="Eliminar {{ $noticia->titulo }}">
    ...
    <livewire:admin.noticias.formulario-noticia />
</div>
